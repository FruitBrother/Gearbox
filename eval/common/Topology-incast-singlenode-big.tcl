...
for { set edge 0 } { $edge < $num_edg_per_pod } { incr edge } {
			for { set index 0 } { $index < $num_host_per_sw } { incr index } {
				...
				$agtagr($pod,$edge,$index,$pod2,$edge2,$index2) set_PCarrival_process  [expr $lambda/($num_hosts - 1)] "$env(HOME)/lining/Gearbox/eval/common/CDF_big.tcl" [expr 17*$w1+1244*$w2] [expr 33*$w1+4369*$w2]
