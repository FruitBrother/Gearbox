...
# Peixuan 12102019: Q01, What is this connection_per_pair, what is connection?
...
# 02262020 Peixuan: Setting bottleneck Node
# 02262020 Peixuan: Actually, we only need one such link, but I still set 4 links where other 3 is idle

set last_pod [expr $num_pod-1]
#set last_edge [expr $num_edg_per_pod-1]
#set last_index [expr $num_host_per_sw-1]

#$ns duplex-link $host($last_pod,$last_edge,$last_index) $edg($last_pod,$last_edge) $bottoneck_rate $bottoneck_delay $bottleneckAlg 

for { set edge 0 } { $edge < $num_edg_per_pod } { incr edge } {
		for { set index 0 } { $index < $num_host_per_sw } { incr index } {
			$ns duplex-link $host($last_pod,$edge,$index) $edg($last_pod,$edge) $bottoneck_rate $bottoneck_delay $bottleneckAlg
		}
	}

# Peixuan02252020: The bottleneck link is the one that connecting edge sw and receiver host
...
# Peixuan02262020: Fix sender as the first pod
set pod 0
...
for { set edge 0 } { $edge < $num_edg_per_pod } { incr edge } {
			for { set index 0 } { $index < $num_host_per_sw } { incr index } {
				...
				$agtagr($pod,$edge,$index,$pod2,$edge2,$index2) set_PCarrival_process  [expr $lambda/($num_hosts - 1)] "$env(HOME)/lining/Gearbox/eval/common/CDF_supersmall.tcl" [expr 17*$w1+1244*$w2] [expr 33*$w1+4369*$w2]
